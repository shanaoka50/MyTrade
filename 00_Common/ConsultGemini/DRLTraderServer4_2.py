...
# OMP: Error #15回避 torchより先にpandasをimportする
import pandas as pd
import torch
import DRLTraderOnLocal as dr # DQN2023用
import DuelNetTraderOnLocalKai as dn # DuelNet用
import DualNetTradingSystem2026_kaiOnLocal2 as ds # DuelNet_2026用
import configparser
import logging
...
def init_nn():
    ...
    if trade_system == 'DQN2023':
        ...
        model_hidden_num = int(request.form['ModelHiddenNum'])
        model_output_num = int(request.form['ModelOutputNum'])
        logger_trader_server.debug('[InitNN]:ModelInputNum=%s, ModelHiddenNum=%s, ModelOutputNum=%s.' %(model_input_num,model_hidden_num,model_output_num))
        ...
        logger_trader_server.debug('[InitNN]:LongPeriod=%s, ShortPeriod=%s, TickvolPeriod=%s.' %(long_period,short_period,tickvol_period))
        ...
        logger_trader_server.debug('[InitNN]:RealSpreadLimit=%s, LCLevel=%s, TPLevel=%s.' %(real_spread_limit,lc_level,tp_level))

        pos_scale = float(request.form['PosScale'])
        pre_trade_weeks = int(request.form['PreTradeWeeks'])
        logger_trader_server.debug('[InitNN]:PosScale=%s, PreTradeWeeks=%s.' %(pos_scale,pre_trade_weeks))
        ...
        trader = ds.Trader(symbol,period,'W',[long_period,short_period,tickvol_period],real_spread_limit,
        ...
        logger_trader_server.warning('[InitNN]:Model %s Not found.'%(model_name))
    ...
    if trade_system == 'DuelNet_2026':
        ...
        # MetaTraderサーバはキプロス時間のためタイムゾーンをキプロス(ニコシア)にする
        # cyprus_tz = pytz.timezone("Asia/Nicosia")
        # trader.ExecPreTrade(datetime.now(cyprus_tz))
        #作成したTraderをdicに格納する。
...
def get_action():
    ...
    if trade_system == 'DQN2023':
      ...
      train_symbol_list = eval(request.form['TrainSymbolList'])
      logger_trader_server.debug('[InitNN]:TrainSymbolList=%s.' %(train_symbol_list))
    elif trade_system == 'DuelNet_2026': # DuelNet_Kaiと同じ
      has_long = float(request.form['HasLong'])
      has_short = float(request.form['HasShort'])
      # fortest
      train_symbol_list = eval(request.form['TrainSymbolList'])
    ...
    # MetaTraderサーバはキプロス時間のためタイムゾーンをキプロス(ニコシア)にする
    # cyprus_tz = pytz.timezone("Asia/Nicosia")
    # dt = dt.replace(tzinfo=cyprus_tz)
    ...
    if trade_system == 'DQN2023':
      ...
      action_mask, sbl, tf = env.GetAvailableAction(has_long, has_short, countdown, timeframe, symbol)
    elif trade_system == 'DuelNet_2026': # profit_lossは使っていない?
      action_mask = env.getAvailableAction(profit_loss,has_long, has_short, countdown)
    ...
    with torch.no_grad():
        if trade_system == 'DuelNet_2026':
            # DuelNet_2026の場合は、#6もbrain(=Agent)で処理する
            acition_index = brain.agnt.DecideAction(dt,input_data, action_mask)
            state_action_values = brain.agnt.main_state_action_values
            masked_action_scores = brain.agnt.main_brain.masked_action_scores
        else:
    ...
    # breinのHiddenStateとCellStateを確認する
    if trade_system == 'DuelNet_2026':
        hidden_state, cell_state = brain.agnt.GetMainHiddenCellState()
    else:
        hidden_state, cell_state = brain.GetHiddenCellState()
    logger_trader_server.debug('[GetAction]:HiddenState %s \n CellState %s' %(hidden_state,cell_state))
